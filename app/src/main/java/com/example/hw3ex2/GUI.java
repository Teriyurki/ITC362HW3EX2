...
//Here is where all the GUI work will be done instead of the xml file.
//We must transfer this information to our controller MainActivity.java
...
public class GUI extends GridLayout{
    private int sideWidth;
    private int sideHeight;
    private Button[][] buttons;
    private TextView status;

    public GUI(Context context, int width, int height, int shadesSideW, int shadesSideH ) {
        super (context);
        sideWidth = shadesSideW;
        sideHeight = shadesSideH;
        setColumnCount(sideWidth);
        setRowCount(sideHeight + 1);

        for( int row = 0; row < sideHeight; row++ ) {
            for (int col = 0; col < sideWidth; col++) {
        ...
        GridLayout.Spec rowSpec = GridLayout.spec( sideHeight, 1 );
        GridLayout.Spec columnSpec = GridLayout.spec( 0, sideWidth );
        GridLayout.LayoutParams lpStatus
                = new GridLayout.LayoutParams( rowSpec, columnSpec );
        status.setLayoutParams( lpStatus );

        // set up status' characteristics
        status.setWidth( sideHeight * width );
